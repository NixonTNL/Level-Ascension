package net.nixontnl.levelascension.skills.logic.melee;
...
public class MeleeSkillManager {
    ...
    static {
        ...
        MOB_XP_MAP.put("minecraft:magma_cube", 15); // optional: scale by size
    ...
    public static void handleMeleeXp(ServerPlayerEntity player, LivingEntity target) {
        PlayerSkillData data = SkillEventHandler.getSkillData(player.getUuid());
        if (data == null) return;

        int xp = getXpForMob(target);
        if (xp > 0) {
            data.addXP(SkillType.MELEE, player, xp);
        }
    }
